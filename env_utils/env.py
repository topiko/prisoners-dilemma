...
def get_pd_env(**kwargs) -> AECEnv[AgentID, ObsType, ActionType | None]:
    return parallel_wrapper_fn(env_fn)(**kwargs)
...
class PD(AECEnv):
    ...
    def __init__(self, max_cycles=15, render_mode=None):
        self.max_cycles = max_cycles
        self.render_mode = "human"
        self.name = "simple_pd_v0"
        self.game = Prisoners_Dilemma()

        num_actions = len(self.game.moves) - 1  # exclude none action
    ...
    def get_other_agent(self, agent: str) -> str:
        idx = 1 - self.agents.index(agent)
    ...
    def reinit(self):
        ...
        self._agent_selector = AgentSelector(agent_order=self.possible_agents)
        ...
        self.observations = {
            agent: [self._none] * len(self.agents) for agent in self.agents
        }
    ...
    def step(self, action):
        ...
        if self._agent_selector.is_last():
            ...
            # other
            self.state[self.get_other_agent(agent)] = self._none
...
if __name__ == "__main__":
    env = get_pd_env(render_mode="nonhuman", max_cycles=21)
    ...
    for _ in range(26):
        ...
        if terminations["player_0"] or truncations["player_0"]:
            obs = env.reset()
            print("Truncated/terminated, resetting environment.")
            break
